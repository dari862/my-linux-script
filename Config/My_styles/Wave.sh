...
change_dock() {
	...
	dconf write /net/launchpad/plank/docks/dock1/offset "80"
...
if [ "$which_panel" == "polybar" ]; then
	...
	# funct STYLE FONT
	change_polybar 'wave' 'Iosevka Nerd Font:size=10;3' && ~/.local/bin/polybar/launch.sh
...
if [ "$(pidof openbox)" ]; then
	...
	if [ "$which_panel" == "polybar" ]; then
		...
		# funct THEME LAYOUT FONT SIZE (Change margin in funct)
		obconfig 'Wave' 'LIMC' 'JetBrains Mono' '9' 'xfce4-menu-color.xml' && openbox --reconfigure
	fi
