...
# funct WALLPAPER
set_wallpaper 'nord.jpg'

if [ "$which_panel" == "polybar" ]; then

	# funct STYLE FONT
	change_polybar 'nordic' 'Iosevka Nerd Font:size=10;3' && ~/.local/bin/polybar/launch.sh
...
if [ -d "$geany_path" ]; then
	# funct SCHEME FONT
	change_geany 'arc' 'Iosevka Custom 10'
fi

# funct THEME ICON CURSOR FONT
change_appearance 'Arc-Dark' 'Arc-Circle' 'Qogirr-dark' 'Noto Sans 9'
