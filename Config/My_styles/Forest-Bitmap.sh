...
change_dock() {
	...
	dconf write /net/launchpad/plank/docks/dock1/offset "95"
	dconf write /net/launchpad/plank/docks/dock1/position "'right'"
...
if [ "$which_panel" == "polybar" ]; then
	...
	# funct STYLE FONT
	change_polybar 'forest-bitmap' 'Terminus:Medium:size=9;2' && ~/.local/bin/polybar/launch.sh
...
if [ "$(pidof openbox)" ]; then
	...
	if [ "$which_panel" == "polybar" ]; then
		...
		# funct THEME LAYOUT FONT SIZE (Change margin in funct)
		obconfig 'Adapta-Bitmap' 'CLM' 'Terminus' '9' 'xfce4-menu-color.xml' && openbox --reconfigure
	fi
fi
